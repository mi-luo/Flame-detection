...
class MPNCOV(nn.Module):
     ...
     """
     def __init__(self, iterNum=3, is_sqrt = False, is_vec = None, input_dim = 2048, dimension_reduction = None):
...
class Covpool(Function):
     ...
     def backward(ctx, grad_output):
         ...
         I_hat = I_hat.to( grad_output.dtype )
         # print(grad_output.dtype, input.dtype)
         batchSize = x.data.shape[0]
         dim = x.data.shape[1]
         h = x.data.shape[2]
         w = x.data.shape[3]
...
class Sqrtm(Function):
     ...
     def backward(ctx, grad_output):
         ...
         # print(input.dtype, A.dtype, ZY.dtype, normA.dtype, Y.dtype, Z.dtype)
         iterN = ctx.iterN
         x = input
         batchSize = x.data.shape[0]
         dim = x.data.shape[1]
