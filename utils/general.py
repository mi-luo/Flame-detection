...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # prediction:[1,18900,85]其中1代表输入图片个数，18900代表网络预测的候选框的个数，85代表(类别数+x,y,w,h,conf)
    """
    #prediction[…, 4]是shape 为torch.Size([1, 18900]) tensor，prediction[…, 4]意义是取所有预测值的第5个值，表示目标框含有目标的概率值，
    整个表达式prediction[…, 4] > conf_thres，返回的是一个与prediction[…, 4]具有相同shape（1, 18900）的tensor，
    其每个值是True或者False，然后把这个tensor赋值给xc，所以xc的shape是（1, 18900），每个值是True或者False，用其值表达每个box的置信度是否大于或者小于conf_thres值
    """

    xc = prediction[..., 4] > conf_thres  # candidates

    # Checks:是check conf和iou是否在0和1之间
    ...
    # 用for循环依次遍历每一张图片，对每一张图片的推理结果进行处理
    # xi是图片的index，其值是0，表示第0张图片，x是图片的推理结果
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # iou修改的地方
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS 原yolov5的！！！
        # i = NMS(boxes, scores, iou_thres, class_nms='XIoU') # XIoU_NMS！！！！！！！！！！！！！
...
def NMS(boxes, scores, iou_thres, class_nms='XIoU'):
    # class_nms=class_nms
    CIoU=XIoU=False
    if class_nms == 'CIoU':
        CIoU=True
    ...
    while B.numel() > 0:
        ...
        keep.append(index)
        if B.numel() == 1: break
        iou = bbox_iou_xiou(boxes[index, :], boxes[B[1:], :], x1y1x2y2=False, XIoU=XIoU, CIoU=CIoU)
